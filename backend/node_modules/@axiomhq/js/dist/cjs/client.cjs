...
class BaseClient extends httpClient.default {
    ...
    constructor(options) {
        if (options.token && token.isAxiomPersonalToken(options.token)) {
     ...
     *
     */
    ingestRaw = async (dataset, data, contentType = exports.ContentType.JSON, contentEncoding = exports.ContentEncoding.Identity, options) => {
        try {
            return await this.client.post(this.localPath + '/datasets/' + dataset + '/ingest', {
                headers: {
